...
public class KeyStoreUtilsTest {
	...
	public void testGetKeyAliases() {
		for (TestConstants.Keystore keystore : TestConstants.Keystore.values()) {
			for (TestConstants.TestPrivateKey privateKey : TestConstants.TestPrivateKey
					.values()) {
				System.out.println("Testing " + keystore + ":" + privateKey);

				String[] keyAliases = KeyStoreUtils.getKeyAliases(privateKey
						.toSignerOptions(keystore));
				...
				// assertTrue(keyList.contains(privateKey.getAlias()) !=
				// privateKey.isExpired());
			}
		}
	}
